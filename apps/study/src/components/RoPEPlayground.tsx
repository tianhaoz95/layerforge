...
function PhasorCircle({ label, angle, color }: { label: string; angle: number; color: string }) {
  const cx = 36
  const cy = 36
  const r = 28
  ...
  return (
    <div className="flex flex-col items-center bg-gray-950/40 p-2.5 rounded-lg border border-gray-900">
      <svg width="72" height="72" viewBox="0 0 72 72">
        ...
        />
        <circle
          cx={vx} cy={vy} r="3" fill={color}
          className="transition-all duration-300 ease-out"
          style={{ filter: `drop-shadow(0 0 4px ${color})` }}
        />
        <circle cx={cx} cy={cy} r="1.5" fill="#374151" />
      </svg>
      <div className="text-[10px] font-mono font-semibold mt-1.5" style={{ color }}>{label}</div>
...
export function RoPEPlayground() {
  ...
  return (
    <div className="relative rounded-2xl border border-gray-800 bg-gray-900/50 p-6 backdrop-blur shadow-2xl">
      ...
      <div className="flex items-center gap-3 mb-6">
        <span className="text-xs font-mono text-gray-400 shrink-0 w-24">Token Position m = {pos}</span>
        <input
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="flex flex-col items-center">
          ...
          <PhasorCircle label="Fast Phasor (θ_0)" angle={qAngleFast} color="#a78bfa" />
        </div>
        <div className="flex flex-col items-center">
          <span className="text-[10px] font-mono text-gray-500 mb-2">Dimension Pair [2, 3] (Low Freq)</span>
          <PhasorCircle label="Slow Phasor (θ_1)" angle={qAngleSlow} color="#22d3ee" />
        </div>
      </div>
      ...
      <div className="rounded-lg bg-gray-950 p-4 border border-gray-800/80 font-mono text-[10px] leading-relaxed text-gray-400">
        <div className="flex items-start gap-2.5">
          ...
          <div>
            ...
            RoPE rotates consecutive pairs of embedding dimensions in 2D sub-spaces at frequency{' '}
            <code className="bg-gray-900 px-1 py-0.5 rounded text-purple-300">θ_i = base^(-2i/d)</code>.
            {' '}As you scrub token position <code className="text-cyan-300">m</code>, the query vector rotates.
            Their dot product depends solely on the relative offset <code className="text-white">m - n</code>.
          </div>
        </div>
      </div>
    </div>
